...
router.get('/auth/google/callback', async (req, res) => {
    try {
        ...
        getIO().to(`user_${state}`).emit('auth:google:callback', code, state);
        res.send(`
    <!DOCTYPE html>
    <html>
    <head>
...
router.get('/auth/facebook/callback', async (req, res) => {
    try {
        const { code, state } = req.query;
        getIO().to(`user_${state}`).emit('auth:facebook:callback', code, state);
        res.send(`
    <!DOCTYPE html>
    <html>
    <head>
...
router.post('/auth/verifyOtpAndLogin', async (req, res) => {
    try {
